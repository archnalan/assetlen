...
using mowt.Service.Domain.Interfaces;
using mowt.Web.Controllers.SmtpClient;
using mowt.API;
using mowt.API.Domain;
using mowt.API.Domain.Interfaces;
using mowt.API.Middlewares;
using Hangfire;
using Mapster;
...
using static mowt.Service.DbServices.AuthorizationDAL;
...
builder.Services.AddDbContext<mowtDbContext>((serviceProvider, options)
    => _ = provider switch
    {
        ...
        //    x => x.MigrationsAssembly("mowt.Sqlite")),
        ...
        "1" => options.UseSqlServer(
           ...
           sqlServerOptions =>
           {
               ...
               sqlServerOptions.MigrationsAssembly("mowt.SqlServer");
           }
       ),
...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.File($"{currentPath.Replace("mowt.API.dll", "").Replace("mowt.API.exe", "")}\\Logs\\Logs.txt", rollingInterval: RollingInterval.Day)
