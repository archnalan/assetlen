...
Each module is a vertical slice — its own `Pages/`, `Components/`, and (later) `Services/` under `mowt.Service`. Routes namespace under the module name (e.g. `/projects`, `/projects/{id}/journal`).
...
Live in [mowt.Shared/Components/](../Components/):
